...
def cm1_test(s, s_next, v, v_next, mu, k, theta, sigma, rho, delta_t):
    a = k * theta
    ...
    cm1 = (7 * sigma**4 * (s_next - s)**6) / (11520 * (-1 + rho**2)**3 * v**5) \
        ...
        + (sigma**2 * (s_next - s)**4) / (96 * (-1 + rho**2)**2 * v**3) \
        - (rho * sigma * (s_next - s)**3 *(v_next - v)) / (24 * (-1 + rho**2)**2 * v**3) \
        ...
        + (rho * (s_next - s) * (v_next - v)**2) / (2 * (-1 + rho**2) * sigma * v**2) \
        - (v_next - v)**3 / (4 * (-1 + rho**2) * sigma**2 * v**2) \
        + (sigma**2 * (s_next - s)**2 - 2 * rho * sigma * (s_next - s) * (v_next - v) + (v_next - v)**2) / (2 * (-1 + rho**2) * sigma**2 * v)
    return cm1

def c0_test(s, s_next, v, v_next, mu, k, theta, sigma, rho, delta_t):
    a = k * theta
    c0 = (7 * sigma**4 * (s_next - s)**4) / (1920 * (-1 + rho**2)**2 * v**4) \
        ...
        + ((-270 * a * rho * (-1 + 2 * rho**2) + sigma * (270 * mu * (-1 + 2 * rho**2) + rho * (97 - 118 * rho**2) * sigma)) * (s_next - s) * (v_next - v)**3) / (1440*(-1 + rho**2)**2*sigma*v**4) \
        + ((360 * a * (-4 + 5 * rho**2) + sigma * (-360 * mu * rho * (-3 + 4 * rho**2) + (-215 + 236 * rho**2) * sigma)) * (v_next - v)**4) / (5760 * (-1 + rho**2)**2 * sigma**2 * v**4) \
        ...
        + (sigma**2*(s_next - s)**2) / (24*(-1 + rho**2)*v**2) \
        + ((12*a + sigma*(-12*mu*rho + sigma))*(v_next - v)**2) / (24*(-1 + rho**2)*sigma**2*v**2) \
...
def c1_test(s, s_next, v, v_next, mu, k, theta, sigma, rho, delta_t):
    c1 = (sigma*(a*rho - mu*sigma)*(s_next - s)) / (12*(-1 + rho**2)*v**2) \
        + ((s_next - s)**2*(60*a**2*(1 + 2*rho**2) + 180*mu**2*sigma**2 + 2*sigma**4 - 2*rho**2*sigma**4 - 60*a*sigma*(6*mu*rho + sigma - rho**2*sigma) - 60*k**2*v**2 + 60*k*rho*sigma*v**2 - 15*sigma**2*v**2)) / (2880*(-1 + rho**2)**2*v**3) \
        ...
        + (1/(2880*(-1 + rho**2)**2*sigma**2*v**3)) * (
            (v_next - v)**2 * (
                180*a**2*(-3 + 4*rho**2) + 60*mu**2*(-7 + 10*rho**2)*sigma**2 - 240*mu*rho*(-1 + rho**2)*sigma**3 - 94*sigma**4 + 190*rho**2*sigma**4 - 96*rho**4*sigma**4 + 60*a*sigma*(mu*(14*rho - 20*rho**3) + (9 - 23*rho**2 + 14*rho**4)*sigma) + 60*k**2*v**2 - 120*k**2*rho**2*v**2 - 60*k*rho*sigma*v**2 + 120*k*rho**3*sigma*v**2 + 15*sigma**2*v**2 - 30*rho**2*sigma**2*v**2)
        ) \
        + (1/(24*(-1 + rho**2)*sigma**2*v)) * (12*a**2 + 12*mu**2*sigma**2 + 2*sigma**4 - 2*rho**2*sigma**4 + 12*mu*(2*k*rho - sigma)*sigma* v + 12*k**2*v**2 - 12*k*rho*sigma*v**2 + 3*sigma**2*v**2 - 12*a*(2*mu*rho*sigma + sigma**2 - rho**2*sigma**2 + 2*k*v - rho*sigma*v)) \
        + (1/(1440*(-1 + rho**2)**2*sigma*v**3))*(
            (s_next - s)*(v_next - v) * (
                -60*a**2*(rho + 2*rho**3) - 180*mu**2*rho*sigma**2 + 120*mu*(-1 + rho**2)*sigma**3 + 180*a*rho*sigma* (2*mu*rho + sigma - rho**2*sigma) + rho*(2*(-1 + rho**2)*sigma**4 + 60*k**2*v**2 - 60*k*rho*sigma*v**2 + 15*sigma**2*v**2)
            )
        )
    return c1

def c2_test(s, s_next, v, v_next, mu, k, theta, sigma, rho, delta_t):
    c2 = -((60*a**2*(-2 + rho**2) - 60*mu**2*sigma**2 + 23*(-1 + rho**2)*sigma**4 + 120*a*sigma*(mu*rho + sigma - rho**2*sigma))/(720*(-1 + rho**2)*v**2))
    return c2

def dv_test(s, s_next, v, v_next, mu, k, theta, sigma, rho, delta_t):
...
print('cm1')
print(int_cm1)
fun = lambda v_next: cm1_test(s, s_next, v, v_next, mu, k, theta, sigma, rho, delta_t)
res = quad(fun, 0, v_max)
print(res[0])

print('c0')
print(int_c0)
fun = lambda v_next: c0_test(s, s_next, v, v_next, mu, k, theta, sigma, rho, delta_t)
res = quad(fun, 0, v_max)
print(res[0])

print('c1')
print(int_c1)
fun = lambda v_next: c1_test(s, s_next, v, v_next, mu, k, theta, sigma, rho, delta_t)
res = quad(fun, 0, v_max)
print(res[0])

print('c2')
print(int_c2)
fun = lambda v_next: c2_test(s, s_next, v, v_next, mu, k, theta, sigma, rho, delta_t)
res = quad(fun, 0, v_max)
print(res[0])

print('Dv')
print(int_Dv)
fun = lambda v_next: dv_test(s, s_next, v, v_next, mu, k, theta, sigma, rho, delta_t)
res = quad(fun, 0, v_max)
print(res[0])