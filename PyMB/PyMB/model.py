    ...
    def simulate_parameters(self, draws=100, params=[], quiet=False, constrain=False):
        ...
        if not self.random:
            self.sdreport = self.TMB.sdreport(self.TMB.model, getJointPrecision=True,
                                              hessian_fixed=get_R_attr_from_NamedList(self.TMB.model, 'he')())
        else:
        ...
        extract_params_mat = self.R.r('''function(obj, nm, keepers) {
            ii <- nm %in% keepers;
            return(obj[ii,ii]);
        }''')

        extract_params_vec = self.R.r('''function(obj, nm, keepers) {
            ii <- nm %in% keepers;
            return(obj[ii]);
        }''')
        ...
        if not self.random:
            joint_prec_full = self.R.r(
                'function(m) { return(1./m) }')(get_R_attr_from_NamedList(self.sdreport, 'cov.fixed'))
            #joint_prec_names = get_R_attr_from_NamedList(self.sdreport, 'par.fixed').names
            joint_prec_names = self.fixed
            joint_prec_dense = extract_params_mat(
                joint_prec_full, joint_prec_names, params)
            joint_prec_R = self.R.r('as')(joint_prec_dense, 'sparseMatrix')
        else:
            joint_prec_full = get_R_attr_from_NamedList(self.sdreport, 'jointPrecision')
            joint_prec_names = self.R.r('row.names')(joint_prec_full)
            joint_prec_R = extract_params_mat(
                joint_prec_full, joint_prec_names, params)
        ...
        # means
        fixed_mean_raw = get_R_attr_from_NamedList(self.sdreport, 'par.fixed')
        # sds
        fixed_sd_raw = get_R_attr_from_NamedList(self.sdreport, 'cov.fixed')
        # names
        fixed_names = self.fixed
        # keep just selected
        fixed_mean = extract_params_vec(fixed_mean_raw, fixed_names, params)
        fixed_sd = extract_params_vec(self.R.r('sqrt')(
            fixed_sd_raw), fixed_names, params)

        # extract random effects
        if self.random:
            # means
            ran_mean_raw = get_R_attr_from_NamedList(self.sdreport, 'par.random')
            # sds
            ran_sd_raw = get_R_attr_from_NamedList(self.sdreport, 'diag.cov.random')
            # names
            ran_names = self.random
            # keep just selected
            ran_mean = extract_params_vec(ran_mean_raw, ran_names,  params)
            ran_sd = extract_params_vec(self.R.r('sqrt')(
