...
Type objective_function<Type>::operator()(){
  ...
  for(int i = 0; i < N; i++){
    ...
    // Gaussian
    case 0:
      nll -= dnorm(y(i), Type(0), exp(h(i) / Type(2)) * sigma_y, true);
      break;
    ...
    case 1:{
      ...
      nll -= (dt(y(i) / normalization, df, true) - log(normalization));
      break; 
      }
    ...
    case 2:{
      ...
      nll -= skew_norm(y(i), alpha(0), omega, xi, true);
      break;
      }
    ...
    case 3:{
      ...
      if(i < (N - 1)){
        ...
        nll -= dnorm(y(i), mu_ + sigma_y * exp(h(i) / Type(2)) * rho * eta, sigma_y * exp(h(i) / Type(2)) * sqrt(Type(1) - rho * rho), true); 
      }
      break;
    }
